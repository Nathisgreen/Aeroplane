...
{
    class TripleShotEnemy : BaseEnemy
    {
        private float shootTime = 0;
        private float ShootTimer;
        private Random aRandom = new Random();

        public TripleShotEnemy(Vector2 location, Texture2D sprite, int movementPattern, int ID = 0)
        ...
        public override void Update(GameTime gameTime, Rectangle PlayerCollsionRect)
        {
            ...
            else
            {
                ...
                Game1.bManager.SpawnBullet(new Vector2(this.getPosition().X + 4, this.getPosition().Y + 12), new Vector2(-12, 3), false);
                Game1.bManager.SpawnBullet(new Vector2(this.getPosition().X + 4, this.getPosition().Y + 12), new Vector2(-12, -3), false);
            }
        }
